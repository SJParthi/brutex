...
//! two [`runner::validate::AnchoredSearchAuthorityProjectionV3`] values.
//!
...
pub(crate) const ANCHORED_SEARCH_LINEAGE_V3_MEMBER_BYTES: usize = 512;
/// Bytes in one receipt-last anchored-search Completion record.
pub(crate) const ANCHORED_SEARCH_LINEAGE_V3_COMPLETION_BYTES: usize = 512;

/// Operator-facing refusal from the paired search-lineage boundary.
pub(crate) type AnchoredSearchLineageV3Refusal = String;

const VERSION: u32 = 3;
const MEMBER_MAGIC: [u8; 16] = *b"BTX-SRCHV3-MEM\0\0";
const COMPLETION_MAGIC: [u8; 16] = *b"BTX-SRCHV3-CMP\0\0";
const MEMBER_DOMAIN: u32 = 1;
const COMPLETION_DOMAIN: u32 = 2;
const SEAL_BYTES: usize = 32;
const MEMBER_PAYLOAD_BYTES: usize = ANCHORED_SEARCH_LINEAGE_V3_MEMBER_BYTES - SEAL_BYTES;
const COMPLETION_PAYLOAD_BYTES: usize = ANCHORED_SEARCH_LINEAGE_V3_COMPLETION_BYTES - SEAL_BYTES;
const SOURCE_AUTHORITY_DOMAIN: &[u8] = b"brutex-anchored-search-lineage-v3-source\0";
const MEMBER_ID_DOMAIN: &[u8] = b"brutex-anchored-search-lineage-v3-member\0";
const PAIR_ID_DOMAIN: &[u8] = b"brutex-anchored-search-lineage-v3-pair\0";
const ORDERED_MEMBER_BYTES_DOMAIN: &[u8] = b"brutex-anchored-search-lineage-v3-ordered-members\0";
const MEMBER_SEAL_DOMAIN: &[u8] = b"brutex-anchored-search-lineage-v3-member-seal\0";
const COMPLETION_SEAL_DOMAIN: &[u8] = b"brutex-anchored-search-lineage-v3-completion-seal\0";
const FILE_GENERATION_DOMAIN: &[u8] = b"brutex-anchored-search-lineage-v3-file\0";
const MEMBER_FILE: &str = "anchored-search-lineage-members-v3.bin";
const COMPLETION_FILE: &str = "anchored-search-lineage-completions-v3.bin";
const LOCK_FILE: &str = "anchored-search-lineage-v3.lock";
...
const _: () =
    assert!(COMPLETION_PAYLOAD_BYTES + SEAL_BYTES == ANCHORED_SEARCH_LINEAGE_V3_COMPLETION_BYTES);
...
pub(crate) struct AnchoredSearchLineageV3Bounds {
    pair_records: u64,
    member_bytes: u64,
    completion_bytes: u64,
}

impl AnchoredSearchLineageV3Bounds {
    ...
    pub(crate) fn new(
    ...
    ) -> Result<Self, AnchoredSearchLineageV3Refusal> {
        for (name, value) in [
        ...
        let required_member_bytes = member_records
            .checked_mul(ANCHORED_SEARCH_LINEAGE_V3_MEMBER_BYTES as u64)
        ...
        let required_completion_bytes = max_pair_records
            .checked_mul(ANCHORED_SEARCH_LINEAGE_V3_COMPLETION_BYTES as u64)
...
impl SearchFamilyV3 {
    fn decode(value: u8) -> Result<Self, AnchoredSearchLineageV3Refusal> {
        match value {
            1 => Ok(Self::Nifty),
            2 => Ok(Self::BankNifty),
            _ => Err(format!(
                "anchored search-lineage family tag {value} is outside the fixed V3 domain"
            )),
        }
    }
}
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
struct MemberRecordV3 {
    block_sequence: u64,
    family: SearchFamilyV3,
...
impl MemberRecordV3 {
    fn from_projection(
        family: SearchFamilyV3,
        projection: &AnchoredSearchAuthorityProjectionV3,
    ) -> Result<Self, AnchoredSearchLineageV3Refusal> {
        let mut value = Self {
            block_sequence: 0,
            family,
    ...
    fn validate(self) -> Result<(), AnchoredSearchLineageV3Refusal> {
        for (name, value) in [
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
struct PreparedPairV3 {
    nifty: MemberRecordV3,
    banknifty: MemberRecordV3,
    pair_id: [u8; 32],
}

impl PreparedPairV3 {
    fn from_opaque(
        nifty: &AnchoredSearchAuthorityProjectionV3,
        banknifty: &AnchoredSearchAuthorityProjectionV3,
    ) -> Result<Self, AnchoredSearchLineageV3Refusal> {
        let nifty = MemberRecordV3::from_projection(SearchFamilyV3::Nifty, nifty)?;
        let banknifty = MemberRecordV3::from_projection(SearchFamilyV3::BankNifty, banknifty)?;
    ...
    fn members(self, block_sequence: u64) -> [MemberRecordV3; 2] {
        [
    ...
    fn completion(
        self,
        block_sequence: u64,
    ) -> Result<CompletionRecordV3, AnchoredSearchLineageV3Refusal> {
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(crate) struct AnchoredSearchLineageV3StructuralReceipt {
    block_sequence: u64,
    pair_id: [u8; 32],
    nifty_member_id: [u8; 32],
    banknifty_member_id: [u8; 32],
...
impl AnchoredSearchLineageV3StructuralReceipt {
...
pub(crate) struct AuthenticatedAnchoredSearchLineageV3 {
    receipt: AnchoredSearchLineageV3StructuralReceipt,
    nifty: MemberRecordV3,
    banknifty: MemberRecordV3,
}

impl AuthenticatedAnchoredSearchLineageV3 {
    pub(crate) const fn structural_receipt(self) -> AnchoredSearchLineageV3StructuralReceipt {
        self.receipt
    }
    ...
    pub(crate) const fn nifty(self) -> AnchoredSearchLineageMemberAuthorityV3 {
        AnchoredSearchLineageMemberAuthorityV3 { member: self.nifty }
    }

    pub(crate) const fn banknifty(self) -> AnchoredSearchLineageMemberAuthorityV3 {
        AnchoredSearchLineageMemberAuthorityV3 {
...
#[derive(Clone, Copy, PartialEq, Eq)]
pub(crate) struct AnchoredSearchLineageMemberAuthorityV3 {
    member: MemberRecordV3,
}

impl AnchoredSearchLineageMemberAuthorityV3 {
...
pub(crate) enum AnchoredSearchLineageV3AuthenticatedCommit {
    Written(AuthenticatedAnchoredSearchLineageV3),
    Reused(AuthenticatedAnchoredSearchLineageV3),
}

impl AnchoredSearchLineageV3AuthenticatedCommit {
    pub(crate) const fn authority(self) -> AuthenticatedAnchoredSearchLineageV3 {
        match self {
...
pub(crate) struct AnchoredSearchLineageV3Ledger {
    root: PathBuf,
    root_file: File,
    ...
    bounds: AnchoredSearchLineageV3Bounds,
    writable: bool,
    receipts: HashMap<[u8; 32], AnchoredSearchLineageV3StructuralReceipt>,
    trailing: Option<TrailingPairV3>,
    member_records: u64,
    completion_records: u64,
}

impl AnchoredSearchLineageV3Ledger {
    ...
    pub(crate) fn structural_receipt(
        &self,
        pair_id: &[u8; 32],
    ) -> Result<Option<AnchoredSearchLineageV3StructuralReceipt>, AnchoredSearchLineageV3Refusal>
    {
        self.lock_file
            .lock_shared()
    ...
    fn structural_receipt_locked_with_after_lookup(
        &self,
        pair_id: &[u8; 32],
        after_lookup: impl FnOnce() -> Result<(), AnchoredSearchLineageV3Refusal>,
    ) -> Result<Option<AnchoredSearchLineageV3StructuralReceipt>, AnchoredSearchLineageV3Refusal>
    {
        self.require_unchanged()?;
        let observed = self.receipts.get(pair_id).copied();
        after_lookup()?;
        self.require_unchanged()?;
        Ok(observed)
    }
    ...
    fn append(
        &mut self,
        prepared: &PreparedPairV3,
    ) -> Result<(bool, AnchoredSearchLineageV3StructuralReceipt), AnchoredSearchLineageV3Refusal>
    {
        if !self.writable {
    ...
    fn require_append_capacity(&self) -> Result<(), AnchoredSearchLineageV3Refusal> {
        let next_pairs = self
            .completion_records
            .checked_add(1)
    ...
    fn require_unchanged(&self) -> Result<(), AnchoredSearchLineageV3Refusal> {
    ...
    fn authenticate(
        ...
        receipt: AnchoredSearchLineageV3StructuralReceipt,
        prepared: &PreparedPairV3,
    ) -> Result<AuthenticatedAnchoredSearchLineageV3, AnchoredSearchLineageV3Refusal> {
        self.require_unchanged()?;
        ...
        self.require_exact_existing(prepared, receipt)?;
        self.require_unchanged()?;
        Ok(AuthenticatedAnchoredSearchLineageV3 {
            receipt,
...
pub(crate) fn persist_anchored_search_lineage_v3(
    root: &Path,
    bounds: AnchoredSearchLineageV3Bounds,
    nifty: &AnchoredSearchAuthorityProjectionV3,
    banknifty: &AnchoredSearchAuthorityProjectionV3,
) -> Result<AnchoredSearchLineageV3AuthenticatedCommit, AnchoredSearchLineageV3Refusal> {
    let prepared = PreparedPairV3::from_opaque(nifty, banknifty)?;
    let mut writer = AnchoredSearchLineageV3Ledger::open_write(root, bounds)?;
    let (written, receipt) = writer.append(&prepared)?;
    drop(writer);
    let mut reopened = AnchoredSearchLineageV3Ledger::open_read(root, bounds)?;
    ...
    Ok(if written {
        AnchoredSearchLineageV3AuthenticatedCommit::Written(authority)
    } else {
        AnchoredSearchLineageV3AuthenticatedCommit::Reused(authority)
    })
}

fn validate_complete_pair(
    sequence: u64,
    members: &[MemberRecordV3; 2],
    completion: CompletionRecordV3,
) -> Result<AnchoredSearchLineageV3StructuralReceipt, AnchoredSearchLineageV3Refusal> {
    ...
    Ok(AnchoredSearchLineageV3StructuralReceipt {
...
fn validate_member_pair(
    sequence: u64,
    members: &[MemberRecordV3; 2],
) -> Result<PreparedPairV3, AnchoredSearchLineageV3Refusal> {
    let nifty = members
        .first()
        .copied()
    ...
    if nifty.family != SearchFamilyV3::Nifty || banknifty.family != SearchFamilyV3::BankNifty {
...
fn encode_member_records(
    members: &[MemberRecordV3; 2],
) -> Result<[[u8; ANCHORED_SEARCH_LINEAGE_V3_MEMBER_BYTES]; 2], AnchoredSearchLineageV3Refusal> {
    Ok([
        encode_member(
            members
                .first()
...
fn encode_members(
    members: &[MemberRecordV3; 2],
) -> Result<Vec<u8>, AnchoredSearchLineageV3Refusal> {
    let records = encode_member_records(members)?;
    let capacity = ANCHORED_SEARCH_LINEAGE_V3_MEMBER_BYTES
        .checked_mul(2)
...
fn encode_member(
    value: &MemberRecordV3,
) -> Result<[u8; ANCHORED_SEARCH_LINEAGE_V3_MEMBER_BYTES], AnchoredSearchLineageV3Refusal> {
    value.validate()?;
    let mut raw = [0_u8; ANCHORED_SEARCH_LINEAGE_V3_MEMBER_BYTES];
...
fn decode_member(
    raw: &[u8; ANCHORED_SEARCH_LINEAGE_V3_MEMBER_BYTES],
) -> Result<MemberRecordV3, AnchoredSearchLineageV3Refusal> {
    let payload = raw
        .get(..MEMBER_PAYLOAD_BYTES)
        .ok_or_else(|| "search-lineage member payload is absent".to_owned())?;
    ...
    let value = MemberRecordV3 {
        block_sequence: reader.u64()?,
        family: SearchFamilyV3::decode(reader.u8()?)?,
...
fn encode_completion(
    value: &CompletionRecordV3,
) -> Result<[u8; ANCHORED_SEARCH_LINEAGE_V3_COMPLETION_BYTES], AnchoredSearchLineageV3Refusal> {
    let mut raw = [0_u8; ANCHORED_SEARCH_LINEAGE_V3_COMPLETION_BYTES];
    let mut writer = FixedWriter::new(
...
fn decode_completion(
    raw: &[u8; ANCHORED_SEARCH_LINEAGE_V3_COMPLETION_BYTES],
) -> Result<CompletionRecordV3, AnchoredSearchLineageV3Refusal> {
    let payload = raw
        .get(..COMPLETION_PAYLOAD_BYTES)
        .ok_or_else(|| "search-lineage Completion payload is absent".to_owned())?;
...
impl<'a> FixedWriter<'a> {
    ...
    fn write(&mut self, value: &[u8]) -> Result<(), AnchoredSearchLineageV3Refusal> {
        let end = self
            .cursor
    ...
    fn u8(&mut self, value: u8) -> Result<(), AnchoredSearchLineageV3Refusal> {
        self.write(&[value])
    }

    fn u32(&mut self, value: u32) -> Result<(), AnchoredSearchLineageV3Refusal> {
        self.write(&value.to_le_bytes())
    }

    fn u64(&mut self, value: u64) -> Result<(), AnchoredSearchLineageV3Refusal> {
        self.write(&value.to_le_bytes())
    }

    fn i64(&mut self, value: i64) -> Result<(), AnchoredSearchLineageV3Refusal> {
        self.write(&value.to_le_bytes())
    }

    fn zeros(&mut self, count: usize) -> Result<(), AnchoredSearchLineageV3Refusal> {
        let end = self
            .cursor
            .checked_add(count)
    ...
    fn zeros_to_end(&mut self) -> Result<(), AnchoredSearchLineageV3Refusal> {
...
impl<'a> FixedReader<'a> {
    ...
    fn take(&mut self, count: usize) -> Result<&'a [u8], AnchoredSearchLineageV3Refusal> {
        let end = self
            .cursor
            .checked_add(count)
    ...
    fn array<const N: usize>(&mut self) -> Result<[u8; N], AnchoredSearchLineageV3Refusal> {
        self.take(N)?
            .try_into()
            .map_err(|_| "search-lineage fixed array width mismatch".to_owned())
    }

    fn u8(&mut self) -> Result<u8, AnchoredSearchLineageV3Refusal> {
        self.take(1)?
            .first()
            .copied()
    ...
    fn u32(&mut self) -> Result<u32, AnchoredSearchLineageV3Refusal> {
        Ok(u32::from_le_bytes(self.array()?))
    }

    fn u64(&mut self) -> Result<u64, AnchoredSearchLineageV3Refusal> {
        Ok(u64::from_le_bytes(self.array()?))
    }

    fn i64(&mut self) -> Result<i64, AnchoredSearchLineageV3Refusal> {
    ...
    fn require_zero_to_end(&mut self, name: &str) -> Result<(), AnchoredSearchLineageV3Refusal> {
...
fn read_member_pair(
    file: &mut File,
    pair_sequence: u64,
) -> Result<[MemberRecordV3; 2], AnchoredSearchLineageV3Refusal> {
    let first = pair_sequence
        .checked_mul(2)
        .ok_or_else(|| "search-lineage member offset overflowed".to_owned())?;
    Ok([
        decode_member(&read_fixed_at::<ANCHORED_SEARCH_LINEAGE_V3_MEMBER_BYTES>(
            file, first,
        )?)?,
        decode_member(&read_fixed_at::<ANCHORED_SEARCH_LINEAGE_V3_MEMBER_BYTES>(
            file,
...
fn read_fixed_at<const N: usize>(
    file: &mut File,
    record: u64,
) -> Result<[u8; N], AnchoredSearchLineageV3Refusal> {
    let width =
...
fn record_count(
    ...
    maximum: u64,
    name: &str,
) -> Result<u64, AnchoredSearchLineageV3Refusal> {
...
fn open_root(
    root: &Path,
) -> Result<(PathBuf, File, PlatformIdentity), AnchoredSearchLineageV3Refusal> {
...
fn named_identity(path: &Path) -> Result<PlatformIdentity, AnchoredSearchLineageV3Refusal> {
...
fn open_child(path: &Path, writable: bool) -> Result<(File, bool), AnchoredSearchLineageV3Refusal> {
...
fn file_generation_with_between_hash_action(
    ...
    between_hashes()?;
    let middle_metadata = file
        .metadata()
        .map_err(|why| format!("cannot restat held search-lineage file after first hash: {why}"))?;
    if MetadataGeneration::of(&middle_metadata) != before || named_identity(path)? != identity {
        return Err(format!(
            "search-lineage file {} changed during bounded generation hashing",
            path.display()
        ));
    }
    let confirmation = hash_held_prefix(file, path, before.len)?;
    let after_metadata = file.metadata().map_err(|why| {
        format!("cannot restat held search-lineage file after second hash: {why}")
    })?;
    if MetadataGeneration::of(&after_metadata) != before || named_identity(path)? != identity {
        return Err(format!(
            "search-lineage file {} changed during bounded generation hashing",
            path.display()
        ));
    ...
    if confirmation != digest {
        return Err(format!(
            "search-lineage file {} changed between bounded generation hashes",
            path.display()
        ));
    }
    Ok(FileGeneration {
...
fn hash_held_prefix(
    ...
    path: &Path,
    length: u64,
) -> Result<[u8; 32], AnchoredSearchLineageV3Refusal> {
    let mut reader = file
        .try_clone()
...
fn sync_directory(file: &File, root: &Path) -> Result<(), AnchoredSearchLineageV3Refusal> {
    file.sync_all()
...
fn combine_with_unlock<T>(
    result: Result<T, AnchoredSearchLineageV3Refusal>,
    released: Result<(), AnchoredSearchLineageV3Refusal>,
) -> Result<T, AnchoredSearchLineageV3Refusal> {
...
mod tests {
    ...
    impl TestRoot {
        fn new(label: &str) -> Self {
            ...
            let path = std::env::temp_dir().join(format!(
                "brutex-search-lineage-v3-{}-{label}-{sequence}",
                std::process::id()
            ));
    ...
    fn bounds() -> AnchoredSearchLineageV3Bounds {
        AnchoredSearchLineageV3Bounds::new(
            8,
            16 * ANCHORED_SEARCH_LINEAGE_V3_MEMBER_BYTES as u64,
            8 * ANCHORED_SEARCH_LINEAGE_V3_COMPLETION_BYTES as u64,
        )
    ...
    fn projection(horizon: u32) -> AnchoredSearchAuthorityProjectionV3 {
        ...
        runner::validate::walk_forward_projected_prepared_anchored_search_v3(
            &bars,
            ExecutionSeries {
                bars: &bars,
    ...
    fn directional_validation(step_paisa: i64) -> AnchoredSearchValidationV3 {
        let mut bars = runner::synthetic::sessions(12);
        for (index, bar) in bars.iter_mut().enumerate() {
            let distance = i64::try_from(index).expect("fixture index fits i64");
            let price = runner::synthetic::BASE
                .checked_add(
                    step_paisa
                        .checked_mul(distance)
                        .expect("fixture price step fits i64"),
                )
                .expect("fixture price fits i64");
            bar.open = price;
            bar.high = price.checked_add(60).expect("fixture high fits i64");
            bar.low = price.checked_sub(60).expect("fixture low fits i64");
            bar.close = price;
        }
        let sweeper = Sweeper::new(Ladder::with_min_hits(1_200).with_ceiling(20_000));
        let mut builder = |slice: &[indicators::Candle]| {
        ...
        .expect("candidate-local side fixture runs through the V3 door")
    }

    fn projections() -> (
        AnchoredSearchAuthorityProjectionV3,
        AnchoredSearchAuthorityProjectionV3,
    ) {
        ...
        *PROJECTIONS.get_or_init(|| (projection(15), projection(16)))
    }

    #[test]
    fn persistence_api_is_directionless_and_v3_projections_cover_local_long_short_and_zero() {
        type PersistV3 = fn(
            &Path,
            AnchoredSearchLineageV3Bounds,
            &AnchoredSearchAuthorityProjectionV3,
            &AnchoredSearchAuthorityProjectionV3,
        ) -> Result<
            AnchoredSearchLineageV3AuthenticatedCommit,
            AnchoredSearchLineageV3Refusal,
        >;
        fn assert_directionless_persistence_api(_: PersistV3) {}
        assert_directionless_persistence_api(persist_anchored_search_lineage_v3);

        for (step, expected) in [
            (3_i64, Direction::Long),
            (-3_i64, Direction::Short),
            (0_i64, Direction::Long),
        ] {
            let opaque = directional_validation(step);
            let projection = opaque
                .search_authority_projection()
                .expect("the V3 projection revalidates its candidate-local side facts");
            assert!(projection.fold_count() > 0);
            let sides: Vec<Direction> = opaque
                .validated()
                .folds
                .iter()
                .filter_map(|fold| fold.chosen_side)
                .collect();
            assert!(
                !sides.is_empty(),
                "fixture must choose at least one candidate"
            );
            assert!(
                sides.iter().all(|side| *side == expected),
                "every chosen candidate must carry the side selected from its own training edge"
            );
        }
    }
    ...
    fn initialize(root: &Path) {
        drop(
            AnchoredSearchLineageV3Ledger::open_write(root, bounds())
    ...
    fn assert_refuses<T>(result: Result<T, AnchoredSearchLineageV3Refusal>, needle: &str) {
    ...
    fn explicit_bounds_cover_pair_member_and_completion_axes() {
        assert_refuses(AnchoredSearchLineageV3Bounds::new(0, 1, 1), "nonzero");
        assert_refuses(
            AnchoredSearchLineageV3Bounds::new(
                2,
                3 * ANCHORED_SEARCH_LINEAGE_V3_MEMBER_BYTES as u64,
                2 * ANCHORED_SEARCH_LINEAGE_V3_COMPLETION_BYTES as u64,
            ),
            "cannot hold",
        );
        assert_refuses(
            AnchoredSearchLineageV3Bounds::new(
                2,
                4 * ANCHORED_SEARCH_LINEAGE_V3_MEMBER_BYTES as u64,
                ANCHORED_SEARCH_LINEAGE_V3_COMPLETION_BYTES as u64,
            ),
            "cannot hold",
        );
    }
    ...
    fn opaque_projections_make_fixed_canonical_non_aliasing_records() {
        ...
        assert_eq!(raw[0].len(), ANCHORED_SEARCH_LINEAGE_V3_MEMBER_BYTES);
        assert_eq!(raw[1].len(), ANCHORED_SEARCH_LINEAGE_V3_MEMBER_BYTES);
        assert_eq!(decode_member(&raw[0]).expect("decode NIFTY"), members[0]);
        assert_eq!(
            decode_member(&raw[1]).expect("decode BANKNIFTY"),
            members[1]
        );
        assert_eq!(members[0].family, SearchFamilyV3::Nifty);
        assert_eq!(members[1].family, SearchFamilyV3::BankNifty);
        assert_ne!(
            members[0].source_authority_id,
            members[1].source_authority_id
        );
        assert_refuses(PreparedPairV3::from_opaque(&nifty, &nifty), "alias");
    }

    #[test]
    fn v3_codecs_pin_magic_version_domains_zero_reserves_and_completion_seals() {
        let (nifty, banknifty) = projections();
        let prepared = PreparedPairV3::from_opaque(&nifty, &banknifty).expect("prepare pair");
        let member = prepared.members(9)[0];
        let member_raw = encode_member(&member).expect("encode member");
        assert_eq!(&member_raw[..16], &MEMBER_MAGIC);
        assert_eq!(
            u32::from_le_bytes(member_raw[16..20].try_into().expect("version")),
            3
        );
        assert_eq!(
            u32::from_le_bytes(member_raw[20..24].try_into().expect("domain")),
            MEMBER_DOMAIN
        );
        assert_eq!(decode_member(&member_raw).expect("decode member"), member);

        let completion = prepared.completion(9).expect("derive Completion");
        let completion_raw = encode_completion(&completion).expect("encode Completion");
        assert_eq!(&completion_raw[..16], &COMPLETION_MAGIC);
        assert_eq!(
            u32::from_le_bytes(completion_raw[16..20].try_into().expect("version")),
            3
        );
        assert_eq!(
            u32::from_le_bytes(completion_raw[20..24].try_into().expect("domain")),
            COMPLETION_DOMAIN
        );
        assert_eq!(
        ...
        let mut noncanonical_member = member_raw;
        noncanonical_member[232] = 1;
        let member_seal = hash_slices(
            MEMBER_SEAL_DOMAIN,
            &[&noncanonical_member[..MEMBER_PAYLOAD_BYTES]],
        );
        noncanonical_member[MEMBER_PAYLOAD_BYTES..].copy_from_slice(&member_seal);
        assert_refuses(decode_member(&noncanonical_member), "member reserve");

        let mut noncanonical_completion = completion_raw;
        noncanonical_completion[192] = 1;
        let completion_seal = hash_slices(
            COMPLETION_SEAL_DOMAIN,
            &[&noncanonical_completion[..COMPLETION_PAYLOAD_BYTES]],
        );
        noncanonical_completion[COMPLETION_PAYLOAD_BYTES..].copy_from_slice(&completion_seal);
        assert_refuses(
    ...
    fn fresh_reopen_authenticates_then_exact_retry_reuses_without_growth() {
        ...
        let (nifty, banknifty) = projections();
        let first = persist_anchored_search_lineage_v3(root.path(), bounds(), &nifty, &banknifty)
            .expect("write and freshly authenticate");
        let first_authority = match first {
            AnchoredSearchLineageV3AuthenticatedCommit::Written(value) => value,
            AnchoredSearchLineageV3AuthenticatedCommit::Reused(_) => {
        ...
        let retry = persist_anchored_search_lineage_v3(root.path(), bounds(), &nifty, &banknifty)
            .expect("exact retry reuses");
        let retry_receipt = match retry {
            AnchoredSearchLineageV3AuthenticatedCommit::Reused(value) => value.structural_receipt(),
        ...
        let reopened = AnchoredSearchLineageV3Ledger::open_read(root.path(), bounds())
    ...
    fn full_synced_member_tail_is_exactly_retryable_but_partial_tail_refuses() {
        ...
        drop(member);
        let retry = persist_anchored_search_lineage_v3(full.path(), bounds(), &nifty, &banknifty)
        ...
        persist_anchored_search_lineage_v3(later.path(), bounds(), &nifty, &banknifty)
            .expect("commit earlier pair");
        let later_nifty = projection(17);
        let later_banknifty = projection(18);
        let later_prepared = PreparedPairV3::from_opaque(&later_nifty, &later_banknifty)
        ...
        drop(later_member);
        let later_retry = persist_anchored_search_lineage_v3(
            later.path(),
            bounds(),
            &later_nifty,
            &later_banknifty,
        ...
        drop(member);
        assert_refuses(
            AnchoredSearchLineageV3Ledger::open_read(partial.path(), bounds()),
    ...
    fn member_and_completion_corruption_fail_closed() {
        ...
        persist_anchored_search_lineage_v3(member_root.path(), bounds(), &nifty, &banknifty)
            .expect("persist member attack fixture");
        overwrite_byte(&member_root.path().join(MEMBER_FILE), 80);
        assert_refuses(
            AnchoredSearchLineageV3Ledger::open_read(member_root.path(), bounds()),
            "member seal mismatch",
        );

        let completion_root = TestRoot::new("completion-corrupt");
        persist_anchored_search_lineage_v3(completion_root.path(), bounds(), &nifty, &banknifty)
            .expect("persist Completion attack fixture");
        overwrite_byte(&completion_root.path().join(COMPLETION_FILE), 80);
        assert_refuses(
            AnchoredSearchLineageV3Ledger::open_read(completion_root.path(), bounds()),
            "Completion seal mismatch",
        );
    }

    #[test]
    fn ragged_member_and_completion_files_refuse_without_prefix_recovery() {
        let member_root = TestRoot::new("ragged-member");
        initialize(member_root.path());
        let mut member = OpenOptions::new()
            .append(true)
            .open(member_root.path().join(MEMBER_FILE))
            .expect("open member file for ragged append");
        member
            .write_all(&[0x55])
            .expect("append ragged member byte");
        member.sync_all().expect("sync ragged member byte");
        drop(member);
        assert_refuses(
            AnchoredSearchLineageV3Ledger::open_read(member_root.path(), bounds()),
            "member file is ragged",
        );

        let completion_root = TestRoot::new("ragged-completion");
        initialize(completion_root.path());
        let mut completion = OpenOptions::new()
            .append(true)
            .open(completion_root.path().join(COMPLETION_FILE))
            .expect("open Completion file for ragged append");
        completion
            .write_all(&[0xaa])
            .expect("append ragged Completion byte");
        completion.sync_all().expect("sync ragged Completion byte");
        drop(completion);
        assert_refuses(
            AnchoredSearchLineageV3Ledger::open_read(completion_root.path(), bounds()),
            "Completion file is ragged",
        );
    }

    #[test]
    fn lock_file_has_an_independent_zero_byte_bound() {
        let root = TestRoot::new("nonempty-lock");
        initialize(root.path());
        let mut lock = OpenOptions::new()
            .append(true)
            .open(root.path().join(LOCK_FILE))
            .expect("open lock file for foreign append");
        lock.write_all(&[1]).expect("append foreign lock byte");
        lock.sync_all().expect("sync foreign lock byte");
        drop(lock);
        assert_refuses(
    ...
    #[test]
    fn mid_hash_and_post_lookup_same_length_mutations_fail_closed() {
        let (nifty, banknifty) = projections();

        let mid_hash = TestRoot::new("mid-hash-mutation");
        persist_anchored_search_lineage_v3(mid_hash.path(), bounds(), &nifty, &banknifty)
            .expect("persist mid-hash fixture");
        let mid_path = mid_hash.path().join(MEMBER_FILE);
        let held = File::open(&mid_path).expect("hold member file for generation hash");
        assert_refuses(
            file_generation_with_between_hash_action(
                &held,
                &mid_path,
                bounds().member_bytes,
                || {
                    overwrite_byte(&mid_path, 80);
                    Ok(())
                },
            ),
            "changed",
        ...
        let post_lookup = TestRoot::new("post-lookup-mutation");
        let receipt =
            persist_anchored_search_lineage_v3(post_lookup.path(), bounds(), &nifty, &banknifty)
                .expect("persist post-lookup fixture")
                .authority()
                .structural_receipt();
        let reader = AnchoredSearchLineageV3Ledger::open_read(post_lookup.path(), bounds())
            .expect("open post-lookup reader");
        let post_path = post_lookup.path().join(MEMBER_FILE);
        assert_refuses(
            reader.structural_receipt_locked_with_after_lookup(&receipt.pair_id(), || {
                overwrite_byte(&post_path, 80);
                Ok(())
            }),
    ...
    fn stale_handles_refuse_same_length_growth_and_path_replacement() {
        ...
        let same_receipt =
            persist_anchored_search_lineage_v3(same_length.path(), bounds(), &nifty, &banknifty)
                .expect("persist same-length fixture")
                .authority()
                .structural_receipt();
        let same_reader = AnchoredSearchLineageV3Ledger::open_read(same_length.path(), bounds())
        ...
        let growth_receipt =
            persist_anchored_search_lineage_v3(growth.path(), bounds(), &nifty, &banknifty)
                .expect("persist growth fixture")
                .authority()
                .structural_receipt();
        let growth_reader = AnchoredSearchLineageV3Ledger::open_read(growth.path(), bounds())
        ...
        let replaced_receipt =
            persist_anchored_search_lineage_v3(replaced.path(), bounds(), &nifty, &banknifty)
                .expect("persist replacement fixture")
                .authority()
                .structural_receipt();
        let replaced_reader = AnchoredSearchLineageV3Ledger::open_read(replaced.path(), bounds())
    ...
    fn resealed_semantic_tamper_remains_structural_and_cannot_mint_runner_authority() {
        ...
        initialize(root.path());
        let (nifty, banknifty) = projections();
        let original = PreparedPairV3::from_opaque(&nifty, &banknifty).expect("prepare real pair");
        ...
        let mut reopened = AnchoredSearchLineageV3Ledger::open_read(root.path(), bounds())
    ...
    fn swapped_family_order_and_foreign_retry_fail_closed() {
        ...
        drop(file);
        let other_bank = projection(17);
        assert_refuses(
            persist_anchored_search_lineage_v3(foreign.path(), bounds(), &nifty, &other_bank),
            "not exact retry",
        );
    }
    ...
    fn symlink_root_and_child_are_refused_without_fallback() {
        ...
        assert_refuses(
            AnchoredSearchLineageV3Ledger::open_read(&root_link, bounds()),
            "real directory",
        );
        ...
        assert_refuses(
            AnchoredSearchLineageV3Ledger::open_read(child.path(), bounds()),
            "regular non-symlink",
        );
    }
}
