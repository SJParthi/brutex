...
export const reasons = ['winning_day_ratio_below_policy_minimum','complete_week_below_policy_minimum_wins','complete_week_above_policy_maximum_losses','losing_day_streak_above_policy_maximum','no_complete_five_session_week','no_eligible_session','missing_expected_session','unmeasured_calendar','invalid_requested_span','invalid_session_order_or_span','session_on_closed_or_excluded_day','invalid_session_count_or_return','exact_arithmetic_overflow','week_output_allocation_refused'];
...
export const policy = () => ({schema_version:1,policy_digest:hex(3),instruments:['NSE-NIFTY','NSE-BANKNIFTY'],minimum_winning_day_numerator:'3',minimum_winning_day_denominator:'5',minimum_week_winning_days:'3',maximum_week_losing_days:'2',maximum_losing_day_streak:'2',pnl_basis:'pessimistic_gross_paisa',zero_days_reset_streak:false,costs_included:false,evaluated_scope:'training_and_later_with_calendar_gap_check',day_rule:'pessimistic_sign',ratio_basis:'eligible_days'});
/** @returns {any} */
...
export function assessment() {
  ...
  return sync({schema_version:1,state:'passed',policy_digest:hex(3),policy:policy(),receipt:{identity:hex(4),completion:hex(5)},qualification:context().qualification,setting_index:'480',combined_qualifies:true,evaluation:null,days_count:'10',weeks_count:'2',evaluated_scope:'training_and_later_with_calendar_gap_check',periods:{training:period(),later:period('11','15'),full}});
}
export function legacy() { return {schema_version:1,state:'not_assessed',policy_digest:null,policy:null,receipt:null,qualification:context().qualification,setting_index:'480',combined_qualifies:null,evaluation:null,days_count:'0',weeks_count:'0',evaluated_scope:null,periods:null}; }
...
export function dayRows(period='full') { return (period==='training'?[4,5,6,7,8]:period==='later'?[11,12,13,14,15]:[4,5,6,7,8,11,12,13,14,15]).map((day,index)=>({index:String(index),day:String(day),pessimistic_paisa:[7,8,14,15].includes(day)?'-100':'400',trades:'2',class:[7,8,14,15].includes(day)?'losing':'winning'})); }
