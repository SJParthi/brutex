...
| AS-12 | **A win inside its own pricing bracket is not the floor — D-0595.** `min_win` took the smallest STRICTLY POSITIVE trade, so one +1 paisa gain set the floor to one paisa and collapsed the 3:1 ratio however large the real winners were. A trade is priced twice — `pess`/`opt` in `grid`, `worst`/`best` on `TradeRow` — and their difference is that trade's own execution uncertainty; a gain no larger than it is a win under one admissible ordering and a loss under another, so it cannot be the evidence a ratio rests on. The threshold is therefore **measured per trade, not declared**: there is no constant and nothing to set wrongly. It is the OPPOSITE test to `Rules::fills_hold`, which demands the uncertainty be large beside the profit | `runner::grid::tests::a_win_inside_its_own_pricing_bracket_is_not_the_smallest_win` — a 20-paisa win spanning 80 must not become the floor, a 300 spanning 100 must, and a zero-bracket fixture still takes the smallest win | ✓ |
| AS-13 | **All FOUR folds moved together, and the suite proved it was needed.** `min_win` is folded from trade rows in four places: `grid::tally_trade` (the `Cell`'s own value, which gained the optimistic reading — `opt` was already in scope at its one production call site), `population_base_evidence_v2::fold_trade_rows` and `institutional_evidence` (both reconcile and refuse on disagreement), and `index_stop_qualification_metrics` (the single-stop route, moved for consistency rather than necessity). **Changing only the first two failed seven tests with "chosen-cell trade detail does not reconcile every P&L, count, extreme, drawdown and streak field with its evaluated cell"** — the reconciliation catching exactly the divergence it exists for. Every other count is untouched, because a scratch really did win and `TradeAggregatesV2::validate` requires `losses == trades - wins`. `runner::outcome::Sides` is deliberately excluded: its `min_win` is over forward MOVES, which carry no second reading and so have no bracket | the identical bracket test in all four files; `cargo test -p cli --lib` 1289 passed / 0 failed and `-p runner --lib` 584 passed / 0 failed | ✓ |
...
| AS-11 | **Two of the four baked findings are NOT closed, and the reason is stated rather than worked around.** The scratch-win collapse (`min_win` is the smallest POSITIVE trade, so one +1 paisa win collapses the 3:1 ratio) is the rule working exactly as written, and repairing it means declaring a magnitude below which a trade is neither a win nor a loss — a number this repository does not have and §3 rule 1 forbids inventing. The day-consistency policy classifies by SIGN alone and vetoes, but its thresholds sit in a unit struct whose digest is bound into run identity, so moving them needs a V2 beside V1 under §3 rule 8 — and what V2 should SAY is a research decision with invented numbers in it. Both are the operator's to declare | recorded in D-0594 rather than fixed; no threshold was invented | — |
