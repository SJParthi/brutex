...
| CIRO-91 | Fifteen saved-result HTTP handlers (not `/trades.json` or `/frontier.json`, which parse inside the blocking task; D-0679) refuse malformed or oversized selectors immediately without a background runtime, returning status 400, valid JSON, a named reason and no rows or saved authority; exact retries retain the response bytes and oversized requests disclose both byte counts | `invalid_saved_queries_return_immediate_named_http_refusals_without_rows` and `oversized_saved_queries_report_both_byte_counts_before_parsing_or_queueing` in `crates/api/src/saved_response_boundary_tests.rs` |
