...
fixture is 40 GB of operator data that CI gate 1 forbids committing, so the
test is `#[ignore]`d on every machine. No CI step passes `--ignored`, so every
CI run lists it as `ignored` and executes none of it — it does not print that
it is skipping, and it proves nothing on CI. Started explicitly with
`--ignored` on a machine without the lake, it panics `MISSING FIXTURE` rather
than report `ok`. Where it has been run — the operator's machine, 2026-08-09 —
it read 1,737 files, 11,526,017 rows, 189 F&O and 1,548 cash/index, and every
per-file digest and the whole-run digest `5bb7cad9d96bb347` were byte-identical
before and after the refusals were added. That is the measurement that says
these refusals cost nothing on sound data; it is not a claim about CI.
...
| C-I-07 | **One bar through `step_known`, the call `Column::build` makes, costs a bounded multiple of a floor timed to hold still.** `C-I-05` times `step`, which skips the known-mask projection, so a slowdown confined to that projection passed every row. Same warmed evaluator and candle as `C-I-05`; the floor is the same one add, two million per trial, minimum of 24 trials, timed after every other row. Budget **5,200** floors. Measured 2026-09-23 on an Apple M4 Pro laptop, six runs: **1275.168, 1251.486, 1246.116, 1285.483, 1267.361** floors at load averages 8 to 13, and **1292.334** at 51, over a floor of 500 to 583 ps; `step_known` read 1.52× to 1.62× `step`. Not measured on CI. Declared by D-0690 | `indicators::bench::a_known_step_stays_within_its_budget` | ✓ |
...
| C-R-05 | **The column build a cash equity runs costs a bounded multiple of a per-bar floor timed to hold still.** `Availability::Present`, which `cli` binds for `Kind::Equity`, turns the VWAP family on; every other `C-R` and `C-I` row builds with `Absent`, so a slowdown confined to VWAP passed all of them. Fixture as `C-R-04`, whose bars carry volume; the bench refuses to time it unless the `Present` column refuses no bar and differs from the `Absent` one. The floor is `C-R-04`'s walk, 2,000 passes per trial, minimum of 15 trials. Budget **14,500** floors. Measured 2026-09-23 on an Apple M4 Pro laptop, eight runs: **3044.550, 3153.084, 3118.809, 3120.959, 3198.445** floors at load averages 11 to 12, and **3220.612, 3023.240, 3635.330** at 42 to 59, over a floor of 318 to 405 ps; the same build with VWAP off read 2014.644 to 2304.560 on that floor. Not measured on CI. Declared by D-0690 | `runner::bench::the_equity_column_build_stays_within_its_budget` | ✓ |
...
| # | Must hold | Proven by | |
|---|---|---|---|
| AU-01 | **Starting the server pulls no data. Only the exact `BRUTEX_AUTOPILOT=run` opt-in enables automatic pulling.** Absence, empty text, whitespace, case variants, `pause` and other near misses stay paused. The environment reader agrees with the pure decision. This is the D-0128 reversal of the old default; pressing Run alone must not consume shared vendor quota. | `api::autopilot::tests::the_boot_default_pulls_nothing_and_only_the_exact_word_run_lets_it_fly` | ✓ |
...
| ST-02 | **A tier names stored constituents; F&O cash eligibility determines which may be swept.** Every member of the four NIFTY tiers is sweepable and named by the swept target exactly when it belongs to the existing F&O cash snapshot (D-0506/D-0508). A tier itself cannot widen eligibility. The two canonical spot-index entries remain separate from this cash research surface, and membership is not point-in-time history. | `api::ingest::tests::a_nifty_tier_target_stores_and_sweeps_exactly_its_fno_members` | ✓ |
...
| ST-05 | `members()` returns `None` for all three targets without a published member file: `Swept`, `Indices` and the union predicate `Everything`. It returns `Some` for every other target rather than manufacturing lists for those three. | `api::ingest::tests::the_three_targets_with_no_published_list_return_none_rather_than_an_invented_one` | ✓ |
...
| R-05 | **The grid count and emitted cells agree with the distinct live and armed trailing axes.** For S stop, T target and R trail rungs the count is `(S+1)[(T+1)(R+1) + T(T+1)/2 × R(R+1)/2]`, giving 625 at four rungs per ladder (D-0249). The shared `grid::variants` calculation governs reservation; the older three-axis and 525-cell formulas are superseded. | `runner::grid::the_cell_count_is_the_product_of_all_three_ladders_and_nothing_reserves_less` and `runner::grid::arming_tests::the_grid_refuses_every_cell_that_cannot_differ` | ✓ |
| R-06 | **Every emitted setting is distinct and exactly one is the baseline.** An armed trail always has a trail to arm; its arm must precede any fixed target, and its trail must be strictly tighter than a simultaneous live trail. These conditions give 625 cells rather than the naive 1,125 with four rungs per ladder (D-0249). | `runner::grid::arming_tests::the_grid_refuses_every_cell_that_cannot_differ` and `runner::grid::arming_tests::exactly_one_cell_is_the_baseline` | ✓ |
...
| ID | Invariant | Proof | ✓ |
|---|---|---|---|
| GPR-02 | **Global Replay V2 retains the requirement for eight selections, sixteen source authority sets and 200 selected streams, but public V1 admission cannot supply the genuine PBO evidence needed to construct that cohort.** The public path therefore refuses. Private generated fixtures prove the 8×25 scheduler, receipt-last publication, fresh reopen, exact retry, retained valid orphans, bounded completion, torn/corrupt companion refusal and same-length stale-writer refusal (D-0619). Manifest identity binds its source set, and replay remains execution-only with no money or VIX claim. These fixtures do not establish a successful public preparation or real-market admission. | `public_v1_authorities_block_global_top25_until_a_genuine_pbo_successor`, `private_controlled_scheduler_preserves_eight_by_twenty_five_algebra_in_memory`, `generated_replay_ledger_round_trips_reuses_and_refuses_stale_or_read_only_writers`, `generated_replay_ledger_refuses_torn_or_corrupt_records_in_every_companion`, `every_global_replay_v2_companion_byte_is_bound_after_outer_resealing`, `generated_replay_ledger_preserves_unacknowledged_orphans_and_enforces_completion_budget`, `manifest_codec_is_new_sealed_and_round_trips_every_authority`, `manifest_identity_refuses_copied_selection_population_and_execution_authority`, `global_scheduler_blocks_direction_instrument_and_timeframe_until_exit`, `same_minute_order_is_priority_then_digest_not_caller_order`, `candidate_identity_binds_universe_ordinal_time_and_refusal_path` in `crates/cli/src/global_replay_v2.rs` | ✓ |
...
| ID | Invariant | Proof | ✓ |
|---|---|---|---|
| AV-07 | **One Admission V3 block is the exact canonical NIFTY-first then BANKNIFTY join of retained Candidate, Base Evidence, Observation, Statistics and Search V4 authorities.** Preparation authenticates the complete Statistics family once, fixed-offset joins every Base row by global/family ordinal and semantic identity, recomputes the canonical Runner V3 decision, and persists every terminal status without converting genuine V3 evidence into Admission V1. Receipt-last append, fresh reopen, exact reuse, valid-zero family, corruption, stale generation, path replacement and every source-identity crosswire fail closed | `bounds_widths_offsets_reserves_and_cross_version_are_exact`, `every_block_source_byte_rekeys_and_codecs_are_deterministic`, `candidate_cardinality_is_independent_of_search_folds_and_all_joins_refuse`, `zero_candidate_block_is_a_complete_receipt_not_a_missing_record`, `successor_projection_reverifies_runner_v3_arithmetic`, `successor_projection_refuses_every_runner_byte_hash_and_status_mutation`, `fresh_reopen_exact_reuse_and_full_orphan_retry_are_durable`, `partial_orphan_corruption_reserve_and_lock_bytes_refuse`, `self_consistent_reseal_is_structural_only_and_stale_generation_refuses`, `symlink_and_path_replacement_are_refused` in `crates/cli/src/population_admission_v3.rs`; `cli::step3_orchestrator::tests::stored_pair_commits_observation_and_statistics_then_reuses_exact_bytes`; focused Admission V3 **10/10**, orchestrator **14/14**, strict CLI library/test Clippy green on 2026-09-01 | ✓ |
| AV-08 | **Finalization V3 can be produced only from the nonconstructible freshly reopened Admission V3 authority and preserves one ordered projection per Admission decision.** Its Finalization identity, ordered-row digest, family counts, terminal counts and Completion are recomputed from the authenticated bulk projection; data rows precede Completion and exact retry is byte-identical. A detached receipt or caller-authored Candidate/Statistics/Search/Base/Admission digest cannot mint the authority | `fixed_codecs_are_v3_only_receipt_last_and_every_identity_byte_is_load_bearing`, `family_global_candidate_statistics_search_base_and_admission_joins_refuse`, `extinct_or_one_sided_candidate_families_remain_complete_receipts`, `explicit_bounds_existing_root_fresh_reopen_and_exact_reuse_are_required`, `full_orphan_exact_retry_completes_once_and_foreign_retry_refuses`, `ragged_torn_corrupt_resealed_reordered_and_structural_forgery_refuse`, `bulk_projection_refuses_stale_replaced_and_late_corruption_without_partial_rows` in `crates/cli/src/population_finalization_v3.rs`; `cli::step3_orchestrator::tests::stored_pair_commits_observation_and_statistics_then_reuses_exact_bytes`; focused Finalization V3 **9/9**, orchestrator **14/14**, strict CLI library/test Clippy green on 2026-09-01 | ✓ |
...
| Selection V6 requires genuine Execution V4 authority, preserves terminal cardinalities and authenticates every fixed byte | `genuine_execution_v4_authority_reaches_v6_and_corrupt_selection_refuses`, `every_byte_is_authenticated_including_unused_padding_and_completion`, `reopen_reuse_and_append_preserve_exact_history_and_capacity` in `crates/cli/src/selection_v6_tests.rs` |
...
| Invariant | Executable proof |
|---|---|
| All 1,024 five-session combinations of winning, losing, flat and no-trade days obey the declared daily ratio, weekly tests and streak limit | `every_five_day_win_loss_flat_no_trade_sequence_obeys_the_declared_policy` in `crates/cli/src/index_consistency_tests.rs` |
| Every nonempty pattern of absent expected sessions is missing evidence, not an invented zero day | `every_missing_position_refuses_instead_of_manufacturing_a_zero_day` in `crates/cli/src/index_consistency_tests.rs` |
| A three-loss streak across two otherwise passing weeks fails; flat/no-trade sessions cannot clear it | `losses_cross_week_boundaries_even_when_both_individual_weeks_pass`; `flat_and_no_trade_days_preserve_a_loss_streak_and_only_a_win_resets_it` in `crates/cli/src/index_consistency_tests.rs` |
| Calendar closures, exclusions and genuine weekend sessions keep their separate meanings | `long_loss_runs_are_counted_exactly_across_holidays_and_year_boundaries`; `existing_excluded_sessions_and_real_weekend_sessions_remain_distinct` in `crates/cli/src/index_consistency_tests.rs` |
| Short/partial weeks do not supply vacuous weekly approval, and unknown calendar facts stay unmeasured | `short_and_partial_weeks_never_pass_the_weekly_test_vacuously`; `missing_calendar_authority_stays_unmeasured_even_when_rows_are_offered` in `crates/cli/src/index_consistency_tests.rs` |
| Malformed order, counts, P&L, spans and overflow refuse before presenting partial work as complete | `malformed_order_counts_returns_spans_and_unexpected_sessions_refuse`; `count_and_both_paisa_overflow_directions_refuse_before_partial_day_updates` in `crates/cli/src/index_consistency_tests.rs` |
| Only the two canonical index families receive the policy; retaining details cannot alter its result identity | `policy_is_index_only_and_week_retention_does_not_change_identity` in `crates/cli/src/index_consistency_tests.rs` |
| Truncation, changed policy fields and forged passing-week claims cannot reopen as valid evidence | `exact_versioned_codecs_refuse_truncation_changed_rules_and_forged_week_claims` in `crates/cli/src/index_consistency_tests.rs` |
...
| Candidate observations include complete long/short pairs and reconcile every event/trade/day even after malicious resealing | `native_roundtrip_retains_complete_program_both_sides_fills_and_all_events`; `publisher_and_cold_reader_refuse_partial_pairs_counts_truncation_and_mutated_rows`; `cold_read_and_warm_pins_refuse_changed_body_receipt_or_owner_without_substitution`; `immutable_receipt_owner_and_byte_identical_replacement_revoke_old_pins`; `reconciliation_rejects_changed_direction_duplicate_trade_links_daily_totals_and_observation_digest`; `reconciliation_checks_underlying_rows_even_after_the_observation_digest_is_updated` in `crates/cli/src/index_stop_store_tests.rs` |
...
| Reader admission cannot be exceeded by an acknowledged checkpoint; exact retry retains the original cursor and selected slots | `pending_restart_preserves_exact_programs_and_never_marks_partial_work_complete`; `byte_node_and_record_admission_happen_before_acknowledgment_and_can_be_raised_for_retry`; `resealed_foreign_alphabet_and_changed_declared_batch_limits_refuse`; `every_selected_timeframe_requires_one_exact_child_and_no_unselected_child`; `canonical_decoder_rejects_bad_flags_padding_truncation_and_hidden_ancestry`; `resealed_history_forks_and_missing_selected_evidence_are_not_completed_research` in `crates/cli/src/index_stop_search_checkpoint_tests.rs` |
...
| V1 and V2 keep identical thresholds and distinct digests, so a freeze test over those two alone cannot observe a policy-dependent classifier | `the_two_policies_share_every_threshold_and_never_share_an_identity` in `crates/cli/src/index_consistency_tests.rs` |
...
| CIRO-43 | Every one-bit change in any byte of a committed Candidate row or completion companion, including headers and recomputed outer seals, refuses the original complete Population authority; exact restoration recovers every original row without changing source bytes | `every_candidate_companion_byte_is_bound_after_outer_resealing` in `crates/cli/src/candidate_universe.rs` |
| CIRO-44 | The actual bar-window route preserves integer prices, real zero versus absent open interest, per-file change provenance, cross-month paging, all sort columns and named missing months; invalid addresses refuse, unreadable months produce partial responses, and an unavailable store root cannot look like empty history | `window_pages_preserve_integer_values_month_gaps_and_change_provenance`, `window_corrupt_months_are_faults_instead_of_silent_missing_months`, `window_malformed_inputs_refuse_instead_of_answering_another_page`, `window_unavailable_store_roots_cannot_be_reported_as_empty_history` in `crates/api/src/bars_window_route_tests.rs` |
| CIRO-45 | Causal comparison rejects absent prefix observations, a shorter whole-series observation set, a missing later suffix or any changed condition mask; its report retains both row counts and the exact disagreement count | `causal_comparison_refuses_missing_short_or_changed_observations` in `crates/cli/src/operator_boundary_tests.rs` |
| CIRO-46 | Every one-bit change at every byte of two committed Pre-Admission V1 or V2 Data/Completion pairs refuses a fresh reader even after recomputing the enclosing seal; V2 covers both zero and nonzero Candidate families, and exact restoration recovers the original audits and values without rewriting storage | `every_v1_pre_admission_byte_is_bound_beyond_its_outer_seal`, `every_v2_pre_admission_byte_binds_zero_and_nonzero_candidate_families` in `crates/cli/src/pre_admission_data.rs` |
| CIRO-47 | A one-bit change at any byte of the Population row, V2 audit, V3 span or V4 calendar companion cannot preserve both the original V4 receipt and its exact rows even after payload resealing; restoring each original companion recovers the original authority and read-only page | `every_population_companion_byte_preserves_the_original_authority_or_refuses` in `crates/cli/src/population.rs` |
| CIRO-48 | The actual scrub route rechecks current stored files and census generations, distinguishes missing/unreadable/count/bounds findings without repairing them, never verifies a missing or corrupt census, keeps feeds separate, and reports every failure count even when the named list is capped at fifty | `scrub_route_uses_current_files_and_never_repairs_disagreements`, `scrub_route_bounds_named_findings_without_truncating_the_failure_count`, `scrub_route_refuses_a_corrupted_counter_instead_of_claiming_an_empty_success` in `crates/api/src/verification_route_tests.rs` |
| CIRO-49 | A real missing bar file yields Missing from the typed storage refusal; an unreadable file whose path includes Missing remains Unreadable, and healthy/count/bounds findings stay distinct without rewriting bytes | `actual_file_findings_use_typed_failures_and_never_diagnostic_keywords` in `crates/pull/src/scrub.rs` |
| CIRO-50 | A gap audit admits calendar peers only when both exchange and segment match, excludes its own vendor/symbol, accepts comparable same-symbol or same-feed peers, and cannot change its response merely because unrelated venue data is stored | `gap_audit_peers_must_match_both_exchange_and_segment` in `crates/api/src/gap_peer_route_tests.rs` |
| CIRO-51 | With all 200 Global Replay V3 witnesses retained, every byte change in the first/last witness and all Candidate/Decision/Money/Completion records refuses fresh authentication under the original capability even after payload resealing; the reader preserves damaged bytes, and exact restoration recovers the same audit and idempotent publication | `replay_v3_record_bytes_remain_bound_after_outer_resealing` in `crates/cli/src/global_replay_v3.rs` |
| CIRO-52 | Explicit invariant proof names bind to the exact tracked source file; every member of a shared-path list must resolve, independent qualified proofs cannot borrow a later path, and malformed references or a zero-reference input refuse | Gate 10 compiles/runs `.github/invariant_paths.rs` and its nine unit cases, including `every_named_list_member_must_exist`, `a_same_named_function_in_another_file_does_not_satisfy_the_proof`, `separate_qualified_proofs_do_not_borrow_a_later_source_path`, `unknown_files_and_noncanonical_paths_refuse` and `malformed_names_and_unclosed_code_spans_refuse`; the command checks nonzero total references |
| CIRO-53 | A named F&O budget halt counts the current contract and every unreached successor; previously held contracts remain untouched and need no unused budget. Incomplete discovery refuses even when readable contracts are absent or already held, adds actual discovery faults without inventing unknown contract counts, and the rendered outcome/reason agrees with the durable audit | `named_chain_budget_halt_counts_the_current_contract_and_unreached_suffix`, `named_chain_already_held_contracts_do_not_require_an_unused_budget`, `unreadable_discovery_is_never_an_empty_or_complete_named_chain`, `complete_discovery_of_no_contracts_remains_an_explicit_empty_result`, `discovery_and_fetch_faults_accumulate_without_inventing_missing_contract_counts`, `an_unstarted_fno_receipt_retains_its_specific_refusal_reason` in `crates/api/src/fno_boundary_tests.rs` |
| CIRO-54 | Named F&O receipts count decoded rows independently from committed source bars, preserve acknowledged writes through later filing failures, retain answered-row counts when a later chunk or budget halts without filing the partial contract, and report exact replays as Empty without claiming no fetch. A failed contract does not hide later writes, limiting displayed reasons never truncates failed-contract totals, and a window after expiry is Empty without implying fetched or held history | `a_named_pull_reports_committed_bars_even_when_its_census_cannot_publish`, `a_named_pull_reuses_exact_bars_without_claiming_a_second_write`, `a_later_named_chunk_refusal_preserves_read_counts_without_filing_a_partial_contract`, `a_named_contract_refusal_does_not_hide_the_following_contracts_committed_bars`, `a_later_named_budget_halt_retains_answered_rows_and_leaves_the_contract_unfiled`, `named_receipts_bound_reasons_without_losing_failed_contract_counts`, `mixed_held_and_replayed_contracts_do_not_claim_that_no_fetch_happened`, `a_window_after_every_discovered_expiry_is_empty_without_claiming_a_fetch` in `crates/api/src/fno_boundary_tests.rs` |
| CIRO-55 | An F&O cell with no common day between its month, requested window and expiry needs no fetch but does not prove held history. The planner counts it in both `complete` and its explicit `outside_window` subset; only fetchable cells probe history and every offered cell remains accounted for | `complete_cells_distinguish_held_history_from_days_never_owed`, `a_cell_with_no_fetchable_day_is_complete_rather_than_asked_for` in `crates/pull/src/fnowork.rs` |
| CIRO-56 | Legacy results and top reports compare only completed rows with trades, rank by the worst-fill total, and select the newest appended row on an exact tie even when its wall clock is older. The listing footer and quality block share that exact selected row; zero-trade and halted rows remain listed but supply neither a winner nor quality. Filtering, display truncation and read failures never substitute another total or write to the ledger | `tied_list_footer_and_quality_choose_the_same_newest_run_as_top`, `unpriced_zero_does_not_beat_a_loss_or_supply_another_runs_quality`, `top_never_selects_the_zero_trade_sentinel_over_a_real_loss`, `an_unpriced_or_halted_only_ledger_has_no_winner_and_no_trade_quality`, `list_filters_and_omitted_rows_do_not_change_the_selected_totals`, `absent_empty_and_corrupt_ledgers_are_read_without_creating_or_repairing_files` in `crates/cli/src/results_report_tests.rs` |
| CIRO-57 | Cold and incremental HTTP top selection share the CLI's completed-trade eligibility, retain actual losing totals instead of an unpriced zero, and keep newest append ties for both global and instrument/feed selections. An all-ineligible ledger renders the same explicit no-winner reason as the CLI; a committed eligible row uses the same renderer | `top_selection_never_ranks_zero_trade_totals_over_losses`, `cached_http_report_uses_the_exact_cli_renderer_after_parent_commit` in `crates/api/src/topjson.rs` |
| CIRO-58 | Trade-quality averages retain the full u64 denominator and signed i64 total, including minimum/maximum boundaries and truncation toward zero; the displayed mean-risk ratio multiplies in a wide intermediate so equal large means still read 1.0x rather than saturating before division | `quality_averages_preserve_large_trade_counts_and_ratio_intermediates` in `crates/cli/src/results_report_tests.rs` |
| CIRO-59 | The public results command resolves the configured owned root and returns the same filtered read-only listing as the root-bound reader; no process-global environment is changed by the fixture | `public_listing_obeys_the_configured_owned_root` in `crates/cli/src/results_report_tests.rs` |
| CIRO-60 | A supplied pricing rate does not authorize pricing after a failed contract landing, even if source bars already committed. A completed landing reaches pricing and retains its actual missing-input reason; a failed census leaves pricing untouched | `named_pricing_requires_a_complete_landing_even_when_source_bars_committed` in `crates/api/src/fno_boundary_tests.rs` |
| CIRO-61 | A source pull can complete while pricing lacks readable spot input; the receipt preserves the actual missing or corrupt read reason even before any row-refusal count is known, without modifying the input or claiming Greek writes | `named_pricing_receipt_preserves_missing_and_corrupt_spot_reasons` in `crates/api/src/fno_boundary_tests.rs` |
| CIRO-62 | Named Greek receipts agree with the committed .grk record count and report a blocked Greek file without claiming an acknowledged write; an unaddressable timeframe refuses before creating files | `named_pricing_receipt_matches_the_greek_file_commit_or_its_refusal`, `greek_filing_refuses_an_unaddressable_rung_without_claiming_a_write` in `crates/api/src/fno_boundary_tests.rs` |
| CIRO-63 | Successful rolling computation is not proof of filing: source-file and Greek-file refusals contribute zero acknowledged Greek rows, successful filing matches the real .grk header, and aggregation preserves those distinct counts | `rolling_pricing_receipt_counts_only_acknowledged_greek_files` in `crates/api/src/fno_boundary_tests.rs` |
| CIRO-64 | Bars, overlays and Greeks retain independent checksum evidence in all six write orders and exact byte-preserving replays; corrupting one proof refuses only that family, and neither reader nor writer creates missing proof for existing sealed derived records or borrows the bar checksum | `record_families_keep_independent_checksums_in_every_write_order_and_exact_replay`, `corrupting_one_integrity_sibling_refuses_only_its_record_family`, `a_missing_derived_checksum_never_borrows_or_rewrites_the_bar_checksum` in `crates/store/src/sidecar_checksum_tests.rs` |
| CIRO-65 | Rolling receipts distinguish decoded rows from newly committed source bars, credit no new writes for an exact replay, and preserve acknowledged bars through a later census publication failure while retaining the failure | `rolling_receipt_keeps_decoded_rows_distinct_from_committed_bars`, `rolling_receipt_does_not_claim_an_exact_replay_as_new_source_writes`, `rolling_receipt_retains_committed_bars_after_a_census_failure` in `crates/api/src/fno_boundary_tests.rs` |
| CIRO-66 | Rolling Greeks use the source ingestion window and session decision, so after-close rows cannot produce Greek records without corresponding accepted source bars | `rolling_greeks_only_describe_bars_the_source_session_accepts` in `crates/api/src/fno_boundary_tests.rs` |
| CIRO-67 | An invalid rolling call-window cap fails before any vendor request; capping displayed rolling reasons at five retains all seven measured failures, decoded rows and the one actual append across exact replies | `rolling_plan_failure_is_explicit_before_any_request`, `rolling_reason_limits_do_not_truncate_failures_or_committed_counts` in `crates/api/src/fno_boundary_tests.rs` |
| CIRO-68 | A decoded rolling answer with no contract strike fails without inventing a contract or writing source bars, while its actual decoded rows remain in the durable receipt | `an_unnameable_rolling_answer_keeps_its_decoded_row_count` in `crates/api/src/fno_boundary_tests.rs` |
| CIRO-69 | A range refused after reading a damaged source reports that no completed result was confirmed, retains the actual file/block refusal, preserves the source bytes and cannot claim that nothing was read or attach successful stored-data provenance | `a_failed_range_never_claims_that_no_source_was_read` in `crates/cli/src/operator_boundary_tests.rs` |
| CIRO-70 | Both current and legacy all-rungs refusal receipts remain browser refusals with their complete text retained and no successful report attached | `a_refused_sweep_is_a_refusal_and_never_a_report` in `crates/api/src/sweeprun.rs` |
| CIRO-71 | Rolling transport charges exactly one shared rate permit for each actual network attempt, including a retried server failure and permanent refusal; a preflight refusal spends no permit | `rolling_requests_spend_one_shared_permit_per_network_attempt`, `rolling_preflight_refusal_never_spends_a_network_permit` in `crates/api/src/fno_boundary_tests.rs` |
| CIRO-72 | A rolling budget halt retains acknowledged source writes and counts each unreached planned cell as a failure without another network request | `rolling_budget_halt_counts_every_unreached_cell_and_retains_prior_writes` in `crates/api/src/fno_boundary_tests.rs` |
| CIRO-73 | Rolling planned-request receipts use the walk's eligible cadence and date chunks; an invalid plan counts no request cells, and an empty answer does not imply an unplanned or failed request | `rolling_plan_and_receipt_count_only_eligible_cadence_chunks`, `rolling_plan_failure_is_explicit_before_any_request` in `crates/api/src/fno_boundary_tests.rs` |
| CIRO-74 | Three refused contract groups in one rolling answer retain their decoded rows and three failures without presenting those failures as three of one requests or claiming source writes | `rolling_group_failures_are_not_reported_as_a_fraction_of_requests` in `crates/api/src/fno_boundary_tests.rs` |
| CIRO-75 | Rolling cadence eligibility resolves the descriptor's first ordinal code, including nonnumeric wire words; an empty code table produces no request or invented ordinal | `rolling_cadence_planning_uses_the_descriptors_ordinal_words` in `crates/api/src/fno_boundary_tests.rs` |
| CIRO-76 | Stored screens calculate support on the retained signal sample and disclose the actual minute-gap dates and removed bars; native and coarse screens preserve the source bytes, and a span with every signal session withheld refuses before an attempt exists | `stored_screens_scale_support_to_retained_bars_and_disclose_holed_sessions`, `stored_screens_refuse_when_every_signal_session_is_withheld` in `crates/cli/src/audited_stored_tests.rs` |
| CIRO-77 | Stored screens retain requested 2%, 5% and 100% thresholds, distinct support identities and exact retry bytes, with matching durable completion status; missing or corrupt execution, signal or prior-reference authority refuses without recording or repairing source files | `stored_screen_support_and_exact_retries_bind_the_actual_sample`, `stored_screens_refuse_missing_or_corrupt_authorities_before_recording` in `crates/cli/src/audited_stored_tests.rs` |
| CIRO-78 | Both public screen entry points preserve non-swept-rung and build-or-feed admission failures without successful provenance or a recorded-result claim; a dirty build cannot reach the supplied feed | `public_screen_admission_preserves_rung_and_build_or_feed_refusals` in `crates/cli/src/audited_stored_tests.rs` |
| CIRO-79 | Candidate signal and execution stream facts require equal timestamp endpoints exactly for a singleton; impossible count/bound pairs remain refused after universe and record resealing, while coherent singleton receipts retain exact bytes | `resealed_candidate_stream_counts_must_agree_with_singleton_timestamp_bounds` in `crates/cli/src/candidate_universe.rs` |
| CIRO-80 | Recomputing the universe identity and full receipt seal cannot hide missing source digests, invalid calendar/cadence bounds, empty or reversed streams, or missing column/grid identities; the unchanged original receipt still decodes exactly | `resealing_candidate_receipts_cannot_hide_invalid_source_components` in `crates/cli/src/candidate_universe.rs` |
| CIRO-81 | Public probe and screen commands use their configured owned store, preserve all input/checksum bytes, disclose the named budget and unvalidated status, and agree with the durable screen identity, support and completion on exact retry; an unstamped build returns the explicit refusal without a results file | `public_generated_probe_and_screen_agree_with_durable_results` in `crates/cli/src/audited_stored_tests.rs` |
| CIRO-82 | A checksum-valid execution record one microsecond off the minute grid refuses both native and coarse screens before a sweep attempt or result is recorded; source and checksum bytes remain unchanged and the exact malformed-execution reason is retained | `stored_screens_reject_off_grid_execution_before_creating_an_attempt` in `crates/cli/src/audited_stored_tests.rs` |
| CIRO-83 | The compiled calendar-policy digest initializes exactly once under concurrent readers, equals an uncached recomputation and preserves its pre-cache 32-byte identity; per-span receipts and runtime source integrity remain separate | `calendar_policy_digest_initializes_once_and_preserves_the_existing_identity` and `calendar_policy_digest_v2_is_canonical_and_not_a_coverage_receipt` in `crates/cli/src/stored.rs` |
| CIRO-84 | Single-month bar requests refuse malformed or reversed supplied day bounds by field name before opening the source; missing bounds remain optional | `single_month_day_bounds_refuse_typos_and_reversed_ranges_before_open` in `crates/api/src/bars_window_route_tests.rs` |
| CIRO-85 | Valid single-month day windows retain inclusive IST dates, exact midnight boundaries, integer values and distinct null/zero open interest without changing source or checksum bytes; valid empty ranges return no rows | `single_month_day_windows_preserve_valid_rows_nulls_and_source_bytes` and `single_day_window_checks_both_ist_midnights_at_microsecond_precision` in `crates/api/src/bars_window_route_tests.rs` |
| CIRO-86 | Walk-forward truncation, noncanonical folds, missing outcomes and overflowing totals cannot become measured successful admission evidence; nonfinite family statistics, invalid probabilities and foreign candidate positions refuse | `validation_refusals_truncation_and_missing_outcomes_never_become_measured_success` and `family_statistics_refuse_nonfinite_values_invalid_probabilities_and_foreign_positions` in `crates/cli/src/institutional_evidence.rs` |
| CIRO-87 | Chosen trade details reconcile every aggregate and retain exact period weakness, concentration and excursions; causal-order errors, overlapping occupancy and negative excursion magnitudes refuse, while empty detail remains unmeasured | `chosen_trade_evidence_reconciles_every_aggregate_and_empty_is_unmeasured` and `chosen_trade_rows_refuse_causal_order_overlap_and_negative_excursions` in `crates/cli/src/institutional_evidence.rs` |
| CIRO-88 | Explicitly refused institutional sources remain refused through the public evidence builder and cannot satisfy even a relaxed numerical admission policy | `explicit_refused_sources_remain_refused_at_the_public_admission_boundary` in `crates/cli/src/institutional_evidence.rs` |
| CIRO-89 | Trade totals and drawdown overflow refuse instead of saturating into admission values; direct-cell counts and sign constraints reject contradictions, and an empty cell retains unmeasured rates and ratios | `trade_aggregate_overflow_refuses_before_a_clamped_value_can_become_evidence` and `direct_cell_measurements_refuse_impossible_counts_and_preserve_unmeasured_ratios` in `crates/cli/src/institutional_evidence.rs` |
| CIRO-91 | Seventeen saved-result HTTP handlers refuse malformed or oversized selectors immediately without a background runtime, returning status 400, valid JSON, a named reason and no rows or saved authority, each under its own refusal envelope and selector grammar (D-0689); exact retries retain the response bytes and oversized requests disclose both byte counts | `invalid_saved_queries_return_immediate_named_http_refusals_without_rows` and `oversized_saved_queries_report_both_byte_counts_before_parsing_or_queueing` in `crates/api/src/saved_response_boundary_tests.rs` |

## The findings ledger on a squash-merged `main` — D-0680

| ID | Invariant | Named proof |
|---|---|---|
| FL-01 | Every commit a `FIXED` or `PARTLY FIXED` row of `docs/11-findings.md` names is an ancestor of `refs/remotes/origin/main`, so a row naming a pull request's branch commit fails before the squash merge instead of on `main` after it; a clone without that ref is refused with the fetch that supplies it, and a copy without `.git` is skipped with the reason printed | `every_named_commit_is_on_main` in `crates/core/tests/findings.rs` |

## Audit follow-ups — D-0681 onward

| # | Must hold | Proven by | |
|---|---|---|---|
| AF-06 | Census-backed GET routes — `/calendar.json` (both branches), `/gaps.json`'s peer vote and `/bars`' segment lookup — read no manifest bytes on a request against an unchanged manifest, and re-read a rewritten manifest exactly once, on whichever route meets the new modified time first; the rewrite is visible in their answers (D-0686, D-0318) | `api::server::census_request_tests::census_backed_get_routes_read_an_unchanged_manifest_once_and_a_rewrite_again` | ✓ |
| AF-07 | **Every answer refuses framing and sniffing, every method must name this listener, and admission runs before the invocation journal.** Every response from either router carries `X-Frame-Options: DENY`, `Content-Security-Policy: frame-ancestors 'none'` and `X-Content-Type-Options: nosniff` exactly once. That includes the fallback, the audited router's `/inspection.json`, `403`, `405` and `413`, and the audit layer's own `503`. `GET` and `HEAD` need exactly one `Host` naming `localhost` or the bound loopback IP on the bound port, as writes do. The operator's reads under either spelling are answered with no, `none`, `same-origin` or `cross-site` fetch metadata. A refused request, or a read of a journaled route whose `Sec-Fetch-Site` is present and is not `same-origin` or `none`, answers `403` before the invocation journal directory exists. Each admitted journaled read adds exactly one record (D-0687) | `api::server::http_admission_tests::every_answer_forbids_framing_and_sniffing`; `api::server::http_admission_tests::the_audit_layers_own_refusal_and_the_plain_router_are_never_framed`; `api::server::http_admission_tests::a_read_must_name_this_listener_and_the_operators_browser_does`; `api::server::http_admission_tests::a_refused_or_cross_site_journaled_request_never_opens_the_journal`; `api::server::http_admission_tests::a_journaled_read_is_refused_only_when_it_names_another_site`; `api::server::tests::only_a_same_origin_write_passes_and_a_local_read_does` | ✓ |
| AF-08 | A tail block whose sidecar entry an interrupted append sealed past the commit is served only when the stored CRC-32C equals the committed bytes followed by 1..k whole records the file holds past the commit, inside the block's nominal end, and a `store.block` warning names it; an overlapping re-offer then re-seals it. A flipped committed byte, a wrong entry, a torn record, an extent past the nominal end, a full block and a mismatch outside the tail block are all still refused, naming the committed extent's checksum. D-0688 | `store::file::a_tail_block_sealed_past_the_commit_by_an_interrupted_append_still_reads`, `store::file::a_damaged_tail_is_refused_even_beside_records_an_interrupted_append_left`, `store::file::a_mismatch_outside_the_tail_block_is_refused_whatever_lies_past_the_commit`, `store::unit::a_tail_checksum_sealed_past_the_commit_is_admitted_only_on_proof`, `store::unit::a_tail_checksum_the_proof_cannot_reach_is_still_refused` | ✓ |
| AF-09 | `/trades.json` and `/frontier.json` parse page, store root and identity before detail admission: a malformed selector is answered at once, with no runtime, as status 400 carrying the exact bytes the blocking task wrote, and is never queued or answered 429 while detail capacity is full | `api::saved_response_boundary_tests::detail_selectors_are_refused_before_admission_with_the_blocking_paths_bytes` | ✓ |
| AF-10 | Gate 8 times the per-bar work a cash-equity run pays and the known-mask projection `Column::build` pays, each against a floor timed over milliseconds with the minimum of several trials kept: `C-R-05` builds the column with `Availability::Present` on bars that carry volume, and refuses to time it unless that column refuses no bar and differs from the `Absent` one; `C-I-07` times `step_known` rather than `step`. Budgets 14,500 and 5,200 floors, sized on eight and six runs on an Apple M4 Pro laptop with roughly 4x left over. `C-R-04` and `C-I-05` keep their floors and budgets. D-0690 | `runner::bench::the_equity_column_build_stays_within_its_budget`, `indicators::bench::a_known_step_stays_within_its_budget` | ✓ |
| AF-11 | **The commit stamp's untracked walk excludes exactly the front-end boundary the tracked comparison excludes.** An untracked file anywhere under the root `web/` directory, such as a new `web/build/` chunk, keeps a clean HEAD stamped with reason "verified clean HEAD". An untracked file in a nested directory only named `web`, in a sibling such as `webpack/`, or a plain root file named `web` still refuses and is named in the reason. D-0691 | `cli::build_provenance::tests::an_untracked_front_end_build_artifact_keeps_the_stamp`; `cli::build_provenance::tests::an_untracked_file_outside_the_front_end_still_refuses` | ✓ |
| AF-12 | **A damaged newest header slot is a torn commit, not a corrupt authority.** Flipping the newest slot's `n_valid` fails that slot's checksum, which is what a crash during the slot write leaves, and a stored screen then reads the previous commit (525 of 600 bars in the fixture) and records it under an identity distinct from the whole month's, without repairing the file. A committed record byte is the corruption the refusal tests damage, and it still refuses before recording | `a_damaged_newest_header_slot_screens_as_the_previous_commit`, `stored_screens_refuse_missing_or_corrupt_authorities_before_recording` in `crates/cli/src/audited_stored_tests.rs` | ✓ |
| AF-13 | **The ZSTD page path every real lake file takes is driven end to end by a test `cargo test --workspace` runs, not only by `#[ignore]`d ones.** A file `parquet`'s writer produced UNCOMPRESSED — dictionary-encoded with one data page per chunk, PLAIN with one row per page and a null open interest beside a real zero, and three dictionary-encoded row groups of 2, 3 and 1 rows with the null in the middle group — is rewritten page by page with `ruzstd`'s own encoder and its footer relabelled ZSTD, and `LakeFile::read_all` on the copy equals the original group for group and bar for bar, each group's row count pinned; the same copy relabelled UNCOMPRESSED is refused as `PageDecode`, and the copy's paisa, null and zero are pinned by value. A `ruzstd`-encoded frame whose first block is a Compressed block (`Block_Type` 2) decodes through `LakePageReader::get_next_page` to exactly the bytes encoded. Mapping `Compression::ZSTD(_)` in `Columns::pages` to a refusal or to `Codec::Uncompressed`, or returning the ZSTD body undecoded, fails the synthetic tests. Before them the two mapping mutations left the whole lake suite green; the undecoded body already failed four `page.rs` unit tests on hand-built Raw and RLE frames, so the gap closed here is the codec mapping and the walk through `LakeFile`, not the decode arm itself. The frames are `ruzstd`'s at `Fastest`, not Polars'; a frame feature only the real encoder emits is still reached only by the `#[ignore]`d `real_lake.rs` | `a_zstd_copy_decodes_to_exactly_what_the_uncompressed_original_does`, `a_zstd_copy_carries_paisa_nulls_and_zeros_to_the_right_rows` in `crates/lake/tests/synthetic.rs`; `a_ruzstd_encoded_compressed_block_decodes_through_the_page_path` in `crates/lake/src/page.rs` | ✓ |
| AF-14 | **A month whose minute count is not `sessions × 375` is walked into the runs it holds, and one whose count matches is decided by the counter.** Against bar files on disk, a January of four daily-proven days — a full session, a 60-minute one, one with a five-minute interior hole (12:00 to 12:04 IST, minutes 720 to 724), one with no minute bars — plus five minutes on a day with no daily bar holds 810 minutes against 1,500 owed and is walked: windows `[(555, 929)]`, `[(555, 614)]` and `[(555, 719), (725, 929)]` owing 375, 60 and 370, `OpenLengthUnmeasured`, and `Closed` for the stray day. In the same derivation a one-session February, and separately a two-session 750-minute month, are decided by the counter with nothing walked. The counter test used to pass no months at all, so it opened no file and read no counter | `a_month_whose_minutes_are_not_sessions_times_375_is_walked_into_its_runs`, `a_month_that_matches_its_counter_is_never_walked` in `crates/api/src/calendar_of.rs` | ✓ |
