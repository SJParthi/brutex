...
| P-54 | **A vendor refusal moves the governor exactly once, and a halved span recovers in a bounded number of requests.** Two defects in one loop. First, `window_async` records the AIMD feedback where the status is read and `api::with_retry` recorded it again on the same shared `Arc`, so one 429 quartered the allowance where AIMD halves it — on a premise its own doc stated and contradicted in the same sentence, *"both entry points already call … which the bars path's `window_async` does not"*. The transport owns the feedback now and no wrapper records, which is what the sibling `laddered` already did. Second, `relax` halves while `tighten_toward_ceiling` added a flat `1`, and the three spans' ceilings differ by four orders of magnitude: a per-second burst also halves the DAY span, which at `+1` then needed 50,000 clean requests to recover against the ~12,996 a 213-instrument pull makes in total. Measured — the allowance walked 100,000 → 50,000 → 25,000 → 12,705 → 6,352 and never returned. The step is `ceiling / RECOVERY_STEPS` now, so recovery costs the same number of requests whatever the ceiling is, and **the second span is untouched** because `5 / 1024` floors to one. **What this does not claim:** the daily ceiling itself is unchanged at 100,000, which the operator adjudicated against two contradicting vendor tables; the governor settling at 6,352 is recorded beside it as evidence, not acted on | `pull::unit::a_halved_day_allowance_recovers_in_requests_a_backfill_actually_makes` — asserts a BOUND, never an exact count, so it cannot become a copy of the implementation · `pull::unit::a_small_rate_ceiling_still_recovers_one_permit_at_a_time` pins the half that did not change · `pull::concurrency::a_throttle_recorded_by_one_thread_binds_every_other` still holds | ✓ |
| P-55 | **A volume counts shares traded, so it is refused at the vendor boundary when it is negative — through all THREE decode doors.** `Bar::counts_are_sane` is `volume >= 0` and it runs in `store::file::survey`, one crate and ~1,500 lines from the vendor, by which point the body is gone: Dhan's `volume: -95342` on ADANIENT decoded cleanly, folded, and died as `ImpossibleCount` at "batch record 2333" — an index into a per-month slice the operator cannot open, printed beside the open-interest null as a second suspect, and costing the instrument-month, its seven derived rungs and every later month in the batch. The floor is on `one_volume` and NOT on `one_number`, because `timestamp` has a legal negative range — `IstMoment::from_epoch_secs` adds the 19,800-second IST offset before testing the sign, so `-19_800..0` is 1970-01-01 and is admitted. **Three doors, and the first patch guarded two:** the parallel-array arm that Dhan actually answers in was missed while `decode_objects` and `decode_positional` were fixed, and the test caught it. **What this does not do:** it does not drop-and-count — a negative volume is evidence the decoder may be reading the wrong column, and `csv.rs` already records the ruling that storing a sent value as absent asserts something false. `csv::decode`'s own volume field has the same hole and is not closed here | `pull::http::a_negative_volume_is_refused_where_the_vendor_still_owns_the_value` — drives a real parallel-array body and pins `one_volume` directly · `pull::http::a_negative_timestamp_is_not_a_negative_count` pins the half that must not change | ✓ |
